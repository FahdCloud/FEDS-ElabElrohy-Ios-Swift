...
struct MarketWebView: View {
    ...
    @State private var showMyStudentMainFromMarketView : Bool = false
    ...
    var body: some View {
        let mediaUrl : String = genralVm.constants.BASE_URL + "/external-platform/shop" + ";userLoginSessionToken=" + userLoginSessionToken
        ...
        NavigationView {
            ...
            .fullScreenCover(isPresented: $showMyStudentMainFromMarketView, content: {
                StudentMainTabView()
            })
            .navigationBarItems(leading: CustomBackButton(){
                clearStatesWithAction(valueState: &showMyStudentMainFromMarketView)
            })
        }
        .onDisappear {
    ...
    private func clearStatesWithAction(valueState: inout Bool) {
        valueState.toggle()
        showMyStudentMainFromMarketView = false
    }
}
