...
@available(iOS 16.0, *)
struct CategoriesGroupsView: View {
    ...
    @State private var showCategoriesHomeFromCategoriesGroups : Bool = false
    ...
    var body: some View {
        NavigationView {
                    ...
                    VStack {
                        ScrollView {
                            ForEach(educationGroupsVm.allEducationalGroupData, id: \.educationalGroupToken) { group in
                                ...
                                .onTapGesture(perform: {
                                    ...
                                    if (group.educationalGroupClosedStateTypeToken ?? "") == Constants().EDUCATION_GROUP_CLOSED_STATE_CLOSED {
                        ...
                        .onAppear(perform: {
                            educationGroupsVm.getGroups(
                                finishedToken: isFinished ? Constants().GROUPS_FINISHED : Constants().GROUPS_UN_FINISHED,
                                educationalCategoryToken: genralVm.categoryToken,
                                refresh:true)
            ...
            .navigationBarItems(leading: CustomBackButton(){
                clearStatesWithAction(valueState: &showCategoriesHomeFromCategoriesGroups)
            }
            )
        ...
        .fullScreenCover(isPresented: $showCategoriesHomeFromCategoriesGroups, content: {
            CategoriesView()
        })
    ...
    private func clearStatesWithAction(valueState: inout Bool) {
        valueState.toggle()
        showCategoriesHomeFromCategoriesGroups = false
    }
